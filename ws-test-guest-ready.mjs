...
async function test() {
  ...
  wsA.on('message', (msg) => {
    ...
    console.log('[PlayerA received]', data.type, data);
    if (data.type === 'room_created') {
       wsB.send(JSON.stringify({ type: 'join_room', code: roomCode, version: '2.3' }));
    }
    if (data.type === 'player_ready_state' && data.guestReady) {
      console.log('Guest is ready, now Host will ready');
  ...
  wsB.on('message', (msg) => {
    const data = JSON.parse(msg.toString());
    console.log('[PlayerB received]', data.type, data);
    if (data.type === 'room_joined') {
      console.log('Guest joined, Guest will ready first');
